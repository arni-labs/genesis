...
**Genesis (formerly temper-git — same project, renamed; never invent
other names) is a Temper-native, GitHub-compatible version-control
...
**Genesis is also the source of truth for TemperPaw and Katagami
apps**: production installs come from Genesis at pinned refs
(`owner/app@hash`), with GitHub as a mirror. Work on Genesis is
therefore on the critical path of every app deploy — when a change
spans repos, merge in dependency order: Genesis → Temper → TemperPaw
→ Katagami.
...
- **DO NOT PUNT.** No phase-0 that defers the hard part, no band-aid
  or temporary fixes. Address ALL review findings and re-run review
  until clean (findings first with severity + file:line, one verdict
  line: PASS or FAIL).
- **Definition of done**: implement fully → live local end-to-end run
  (real `git`/`gh` against a running Genesis) → merge + deploy →
  verify live in the deployed system (Railway), using **Datadog** for
  verification and diagnosis → hand over PR links, merge commits,
  deployment links, live test results, residual risks.
- When something "keeps happening" or "didn't use to happen": find
  **what changed**, fix the root cause, and explain the causal story.
  Never stack fixes on fixes. Surface every error and policy denial
  to the human channel — silent failure is itself a bug.
