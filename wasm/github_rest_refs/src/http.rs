...
/// 204 for DELETE endpoints — GitHub sends no body with it.
pub(crate) fn respond_no_content(http: &InboundHttp) -> Result<Value, String> {
    http.submit_response_head(204, &[])
        .map_err(|e| format!("submit_response_head: {e}"))?;
    let writer = http.response_body();
    writer
        .finish()
        .map_err(|e| format!("response_body close: {e}"))?;
    Ok(json!({ "status": 204 }))
