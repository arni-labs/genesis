...
temper_module! {
    fn run(ctx: Context) -> Result<Value> {
        ...
        let objects = match pack_bytes {
            Some(pack_bytes) => {
                parse_pack_objects(&api_base, &blob_endpoint, &repository_id, &pack_bytes)?
            }
            None => Vec::new(),
        };
...
fn parse_pack_objects(
    ...
    while let Some(obj) = parser
        .next_object_with_ref_delta_base(|sha| {
            fetch_existing_delta_base(api_base, blob_endpoint, repository_id, sha)
...
fn fetch_existing_delta_base(
    ...
    for (kind, set) in [
        ...
        if let Some(data) =
            fetch_existing_object_body(api_base, blob_endpoint, repository_id, set, sha)?
        {
...
fn fetch_existing_object_body(
    ...
    let body = get_streamed_text(&url, &format!("fetch {set}({sha})"))?;
...
fn field_overflow_blob_key(value: &Value) -> Result<Option<String>, String> {
    ...
    if encoding != "json" {
        return Err(format!(
            "field-overflow blob-ref encoding {encoding:?} is not supported"
        ));
    }
...
fn get_streamed_text(url: &str, label: &str) -> Result<String, String> {
    let (request_body, mut response_body, response_head) =
        streaming_call("GET", &url, &[]).map_err(|e| format!("{label} stream begin: {e}"))?;
    request_body
        .finish()
...
mod tests {
    ...
    fn canonical_body_resolves_field_overflow_ref() {
        ...
        let serialized = serde_json::to_string(&Value::String(canonical)).expect("serialize");
        let blob_ref = json!({
